...
function buildRouteMapHTML(
  ...
  // OSRM 경유지 좌표 문자열 (lng,lat 순서)
  const osrmCoords = path.map(n => `${n.lng},${n.lat}`).join(';');
  const osrmFetchJS = path.length >= 2 ? `
    var routeLayer = null;
      ...
      .then(function(data){
        if(data.routes && data.routes[0]){
          ...
          routeLayer = L.geoJSON(data.routes[0].geometry, {
            style: { color: '#2B6CB0', weight: 5, opacity: 0.9 }
          }).addTo(map);
        }
      })
      .catch(function(){
        // OSRM 실패 시 직선으로 fallback
        var coords = [${path.map(n => `[${n.lat},${n.lng}]`).join(',')}];
        L.polyline(coords, {color:'#2B6CB0',weight:4,opacity:0.85,dashArray:'8,4'}).addTo(map);
      });
  ` : '';
