...
// 웹에서 Alert이 동작하지 않는 문제 보완
const showAlert = (title: string, message: string, buttons?: any[]) => {
  if (Platform.OS === 'web') {
    if (buttons && buttons.length > 1) {
      const ok = window.confirm(`${title}\n\n${message}`);
      if (ok) {
        const confirmBtn = buttons.find((b: any) => b.style !== 'cancel' && b.onPress);
        confirmBtn?.onPress?.();
      }
    } else {
      window.alert(`${title}\n\n${message}`);
      const btn = buttons?.find((b: any) => b.onPress);
      btn?.onPress?.();
    }
  } else {
...
export const CreatePackageScreen: React.FC = () => {
  ...
  const handleCreateSchedule = () => {
    if (!title.trim()) {
      showAlert('알림', '여행 제목을 입력해주세요.');
      return;
    }
    if (!destination.trim()) {
      showAlert('알림', '여행지를 입력해주세요.');
      return;
    }
    ...
    const newPlanInfo: PlanInfo = {
      title, dest: destination, start: startDateStr, end: endDateStr,
      days, people: people || '2', budget: '', transport: transport || '자가용', memo: memo || '',
    };
    setPlanInfo(newPlanInfo);
    if (useAI) {
      showAlert('🤖 AI 일정 생성', 'AI가 자동으로 일정을 생성합니다.', [
  ...
  const generateAISchedule = async (info: PlanInfo) => {
    ...
    for (let i = 0; i < info.days; i++) {
      ...
      const slots: ScheduleSlot[] = [
        { id: `${i}_1`, start_time: '09:00', name: i === 0 ? `${info.dest} 도착` : '호텔 출발', city: info.dest, category: 'accommodation', duration: 30 },
        { id: `${i}_2`, start_time: '10:00', name: places.morning[i % places.morning.length], city: info.dest, category: 'attraction', duration: 120 },
        { id: `${i}_3`, start_time: '12:30', name: places.lunch[i % places.lunch.length], city: info.dest, category: 'restaurant', duration: 60 },
        { id: `${i}_4`, start_time: '14:30', name: places.afternoon[i % places.afternoon.length], city: info.dest, category: 'cafe', duration: 120 },
        { id: `${i}_5`, start_time: '17:30', name: `${info.dest} 석양`, city: info.dest, category: 'activity', duration: 60 },
        { id: `${i}_6`, start_time: '19:00', name: places.dinner[i % places.dinner.length], city: info.dest, category: 'restaurant', duration: 90 },
      ];
      schedules.push({ day: i + 1, date: d.toISOString().slice(0, 10), slots });
    }
    setDaySchedules(schedules);
    setShowSchedule(true);
    addNotification({ title: '🤖 AI 일정 생성', message: `${personalityName} 기반 ${info.days}일 일정 생성 완료`, type: 'info' });
  };
  ...
  const addSlot = (dayIndex: number) => {
    ...
    updated[dayIndex].slots.push({ id: Date.now().toString(), start_time: '09:00', name: '새 일정', city: planInfo?.dest || destination, category: 'attraction' });
  ...
  const handleSavePlan = async () => {
    if (daySchedules.every(d => d.slots.length === 0)) {
      showAlert('알림', '일정을 한 개 이상 추가해주세요!');
      return;
    }
    if (!planInfo) { showAlert('오류', '일정 정보가 없습니다.'); return; }
    try {
      const planData = { id: Date.now().toString(), info: planInfo, schedule: daySchedules, savedAt: new Date().toISOString(), createdAt: new Date().toISOString() };
      const saved = await AsyncStorage.getItem('travel_plans');
      ...
      showAlert('성공', '저장되었습니다!', [{ text: '확인', onPress: () => { loadSavedPlans(); setActiveTab('library'); } }]);
    } catch { showAlert('오류', '저장 실패'); }
  };

  const handleClearPlan = () => {
    showAlert('초기화', '일정을 초기화하시겠습니까?', [
      { text: '취소', style: 'cancel' },
      { text: '초기화', style: 'destructive', onPress: () => {
        setShowSchedule(false); setDaySchedules([]); setPlanInfo(null);
        setTitle(''); setDestination(''); setStartDate(new Date()); setEndDate(new Date());
        setStartDateText(''); setEndDateText(''); setPeople(''); setTransport(''); setMemo(''); setUseAI(false);
  ...
  const loadSavedPlan = (plan: any) => {
    showAlert('불러오기', `'${plan.info.title}' 일정을 불러오시겠습니까?`, [
      { text: '취소', style: 'cancel' },
      { text: '불러오기', onPress: () => {
        const info = plan.info;
        setPlanInfo(info); setTitle(info.title); setDestination(info.dest);
        setStartDate(new Date(info.start)); setEndDate(new Date(info.end));
        setStartDateText(info.start); setEndDateText(info.end);
        setPeople(info.people || '2'); setTransport(info.transport || '자가용'); setMemo(info.memo || '');
        setDaySchedules(plan.schedule); setShowSchedule(true); setActiveTab('create');
        addNotification({ title: '불러오기', message: `'${info.title}' 불러오기 완료`, type: 'info' });
      }}
    ]);
  };
              ...
              <View>
                <View style={styles.scheduleHeader}>
                  ...
                  <Text style={styles.scheduleMeta}>
                    📍 {planInfo?.dest || destination} · 📅 {planInfo?.start || formatDate(startDate)} ~ {planInfo?.end || formatDate(endDate)} ({planInfo?.days || 0}일) · 👥 {planInfo?.people || people}명 · 🚗 {planInfo?.transport || transport}
                  </Text>
                    ...
                    <View style={styles.dayBody}>
                      {day.slots.map((slot) => (
                        <View key={slot.id} style={styles.slotItem}>
                          <TextInput style={styles.timeInput} value={slot.start_time} onChangeText={(text) => updateSlotTime(dayIndex, slot.id, text)} placeholder="09:00" />
                          <Text style={styles.slotIcon}>{getCategoryEmoji(slot.category)}</Text>
                          <View style={styles.slotInfo}>
                            <TextInput style={styles.slotNameInput} value={slot.name} onChangeText={(text) => updateSlotName(dayIndex, slot.id, text)} placeholder="장소 이름" />
            ...
            {savedPlans.length === 0 ? (
              ...
              savedPlans.map((plan) => (
                <TouchableOpacity key={plan.id} style={styles.planCard} onPress={() => loadSavedPlan(plan)} activeOpacity={0.7}>
                  ...
                  <Text style={styles.planMeta}>📍 {plan.info?.dest || plan.destination} · 📅 {plan.info?.start || plan.startDate} ~ {plan.info?.end || plan.endDate}</Text>
                  <Text style={styles.planDetail}>👥 {plan.info?.people || plan.people}명 · 🚗 {plan.info?.transport || plan.transport}</Text>
                  {plan.schedule && <Text style={styles.planSlots}>📋 총 {plan.schedule.reduce((sum: number, day: any) => sum + day.slots.length, 0)}개 일정</Text>}
                  <TouchableOpacity style={styles.deletePlanBtn} onPress={(e) => {
                    e.stopPropagation();
                    showAlert('삭제', '이 일정을 삭제하시겠습니까?', [
                      { text: '취소', style: 'cancel' },
            ...
            <View style={styles.modalOverlay}>
              <View style={styles.modalContent}>
                <View style={styles.modalHeader}>
                  <TouchableOpacity onPress={() => setShowStartPicker(false)}><Text style={styles.modalButton}>취소</Text></TouchableOpacity>
                  <Text style={styles.modalTitle}>출발일 선택</Text>
                  <TouchableOpacity onPress={() => setShowStartPicker(false)}><Text style={[styles.modalButton, styles.modalButtonPrimary]}>완료</Text></TouchableOpacity>
                </View>
                <DateTimePicker value={startDate} mode="date" display="spinner" onChange={onStartDateChange} minimumDate={new Date()} style={styles.datePicker} />
              </View>
            </View>
          </Modal>
        ) : (
        ...
        Platform.OS === 'ios' ? (
          <Modal transparent animationType="slide" visible={showEndPicker} onRequestClose={() => setShowEndPicker(false)}>
            <View style={styles.modalOverlay}>
              <View style={styles.modalContent}>
                <View style={styles.modalHeader}>
                  <TouchableOpacity onPress={() => setShowEndPicker(false)}><Text style={styles.modalButton}>취소</Text></TouchableOpacity>
                  <Text style={styles.modalTitle}>도착일 선택</Text>
                  <TouchableOpacity onPress={() => setShowEndPicker(false)}><Text style={[styles.modalButton, styles.modalButtonPrimary]}>완료</Text></TouchableOpacity>
                </View>
                <DateTimePicker value={endDate} mode="date" display="spinner" onChange={onEndDateChange} minimumDate={startDate} style={styles.datePicker} />
              </View>
            </View>
          </Modal>
        ) : (
...
const styles = StyleSheet.create({
  ...
  dateInputContainer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderWidth: 1.5, borderColor: '#E0E0E0', borderRadius: 10, backgroundColor: '#fff', paddingLeft: 14, paddingVertical: 14 },
  dateInputText: { flex: 1, fontSize: 15, color: '#1A1A1A', fontWeight: '500' },
  calendarBtn: { paddingHorizontal: 12 },
  ...
  aiOptionBox: { padding: 16, backgroundColor: '#F0F4FF', borderWidth: 2, borderStyle: 'dashed', borderColor: Colors.primary, borderRadius: 12, marginBottom: 16 },
  checkboxContainer: { flexDirection: 'row', alignItems: 'center' },
  checkbox: { width: 24, height: 24, borderRadius: 6, borderWidth: 2, borderColor: Colors.primary, marginRight: 12, justifyContent: 'center', alignItems: 'center', backgroundColor: '#fff' },
  checkboxChecked: { backgroundColor: Colors.primary },
  checkmark: { color: '#fff', fontSize: 14, fontWeight: '800' },
  aiTextContainer: { flex: 1 },
  aiTitle: { fontSize: 15, fontWeight: '700', color: '#1A1A1A', marginBottom: 2 },
  aiSubtitle: { fontSize: 12, color: '#666' },
  scheduleHeader: { backgroundColor: '#fff', padding: 20, borderRadius: 16, marginBottom: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 2 },
  scheduleTitle: { fontSize: 22, fontWeight: '800', color: '#1A1A1A', marginBottom: 8 },
  scheduleMeta: { fontSize: 13, color: '#666', lineHeight: 20, marginBottom: 12 },
  scheduleActions: { flexDirection: 'row', gap: 10 },
  actionBtn: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 8, borderWidth: 1.5, borderColor: '#FF6B6B', gap: 6 },
  actionBtnText: { fontSize: 14, fontWeight: '600' },
  actionBtnPrimary: { backgroundColor: Colors.primary, borderColor: Colors.primary },
  actionBtnTextPrimary: { color: '#fff', fontSize: 14, fontWeight: '700' },
  dayCard: { backgroundColor: '#fff', borderRadius: 16, marginBottom: 16, overflow: 'hidden', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 2 },
  dayHeader: { backgroundColor: Colors.primary, padding: 14 },
  dayTitle: { color: '#fff', fontSize: 16, fontWeight: '700' },
  dayBody: { padding: 16 },
  slotItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#F0F0F0', gap: 10 },
  timeInput: { width: 55, fontSize: 13, color: '#333', fontWeight: '600', borderWidth: 1, borderColor: '#E0E0E0', borderRadius: 6, padding: 6, textAlign: 'center' },
  slotIcon: { fontSize: 20 },
  slotInfo: { flex: 1 },
  slotNameInput: { fontSize: 15, fontWeight: '600', color: '#1A1A1A', borderBottomWidth: 1, borderBottomColor: '#E0E0E0', paddingBottom: 2, marginBottom: 4 },
  slotMeta: { fontSize: 12, color: '#999' },
  slotDuration: { fontSize: 11, color: '#BBB', marginTop: 2 },
  addSlotBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 12, marginTop: 8, borderWidth: 1.5, borderStyle: 'dashed', borderColor: Colors.primary, borderRadius: 10, gap: 8 },
  addSlotText: { color: Colors.primary, fontSize: 14, fontWeight: '600' },
  libraryHeader: { marginBottom: 20 },
  libraryTitle: { fontSize: 22, fontWeight: '800', color: '#1A1A1A', marginBottom: 4 },
  librarySubtitle: { fontSize: 14, color: '#666' },
  emptyState: { alignItems: 'center', paddingVertical: 60 },
  emptyIcon: { fontSize: 48, marginBottom: 16 },
  emptyText: { fontSize: 15, color: '#999', textAlign: 'center', lineHeight: 24 },
  planCard: { backgroundColor: '#fff', borderRadius: 16, padding: 18, marginBottom: 14, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 2, position: 'relative' },
  planTitle: { fontSize: 17, fontWeight: '700', color: '#1A1A1A', marginBottom: 6 },
  planMeta: { fontSize: 13, color: '#666', marginBottom: 4 },
  planDetail: { fontSize: 13, color: '#666', marginBottom: 4 },
  planSlots: { fontSize: 12, color: Colors.primary, fontWeight: '600', marginTop: 4 },
  deletePlanBtn: { position: 'absolute', top: 16, right: 16, padding: 4 },
  ...
  modalButton: { fontSize: 16, color: '#666', fontWeight: '600' },
